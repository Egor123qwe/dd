...
func (h *handler) newSignInHandler() gin.HandlerFunc {
	return ginExtend.NewHandler(respProcessor.JsonRespSender)(
		func(c *gin.Context) (any, []string, error) {
			...
			resp, err := h.signInUC.SignIn(c.Request.Context(), req.Convert())
			if err != nil {
				h.log.Err(err).Str("email", req.Email).Msg("failed to sign in")
				return nil, nil, err
			}

			return dto.ToSignInResp(resp), nil, nil
		},
	)
}
