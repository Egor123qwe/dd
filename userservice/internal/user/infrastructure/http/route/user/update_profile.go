...
func (h *handler) newUpdateUserProfileHandler() gin.HandlerFunc {
	return ginExtend.NewHandler(respProcessor.JsonRespSender)(
		func(c *gin.Context) (any, []string, error) {
			...
			err = h.updateUserProfileUC.UpdateUserProfile(c.Request.Context(), *permit, req.Convert(userID))
			if err != nil {
				h.log.Err(err).Int("user_id", userID).Msg("failed to update user profile")
				return nil, nil, err
			}
